...
type topic_obj = { value : string; creator : user; last_set : timestamp }
...
type field_obj = { title : string option; value : string; short : bool }
...
type channel_leave_obj = { not_in_channel : bool option }
...
type chat_obj = { ts : timestamp; chat : chat; text : string option }

type emoji = string * string
(** A single emoji. *)

type chat_close_obj = { no_op : bool option; already_closed : bool option }

type groups_invite_obj = { already_in_group : bool option; group : group_obj }
(** Response to a channel invite. *)

type groups_open_obj = { no_op : bool option; already_open : bool option }
...
type stars_list_obj = { items : Yojson.Safe.t list; paging : paging_obj }
...
val api_test :
  ...
  ?error:string ->
  unit ->
  [ `Success of Yojson.Safe.t | api_error ] Lwt.t
...
val conversations_list :
  ?exclude_archived:bool ->
  session ->
  [ `Success of conversation_obj list | parsed_auth_error ] Lwt.t
...
val team_info :
  session -> [ `Success of team_obj | parsed_auth_error | bot_error ] Lwt.t
...
val users_list :
  session -> [ `Success of user_obj list | parsed_auth_error ] Lwt.t
(** Lists all users in a Slack team. *)

val users_set_active :
  session -> [ `Success | parsed_auth_error | bot_error ] Lwt.t
(** Marks a user as active. *)

val users_set_presence :
  session -> presence -> [ `Success | parsed_auth_error | presence_error ] Lwt.t
